...
<html lang="es">
    ...
    %>
        ...
        <c:if test="${not empty ProductosDes}">
            <div id="carouselExample" class="carousel slide">
                <div class="carousel-inner">
                    <c:forEach var="cat" items="${catDes}" varStatus="catStatus">
                        <c:forEach var="c" items="${ProductosDes}" varStatus="prodStatus">
                            <c:if test="${c.getIdCategoria() == cat.getId()}">
                                <c:set var="descuento" value="${cat.getOfertas()}"/>
                                <c:set var="precioOriginal" value="${c.getPrecio()}"/>
                                <c:set var="precioDescuento" value="${precioOriginal - (precioOriginal * descuento / 100)}"/>

                                <div class="carousel-item ${prodStatus.index == 0 ? 'active' : ''}">
                                    <div class="row justify-content-center align-items-center">
                                        <div class="col-md-6 d-flex justify-content-center align-items-center">
                                            <div class="custom-image-container">
                                                <img src="${c.getFoto()}" class="custom-product-image" alt="Producto">
                                            </div>
                                        </div>
                                        <div class="col-md-6 d-flex flex-column justify-content-center">
                                            <div class="custom-product-info-container">
                                                <p class="custom-product-discount-text">Producto en ${descuento}% de Descuento</p>
                                                <h2 class="custom-product-name">${c.getNombre()}</h2>
                                                <p class="custom-product-description">${c.getDescripcion()}</p>
                                                <p class="custom-product-old-price">Antes: $${precioOriginal}</p>
                                                <p class="custom-product-price">Ahora: $${precioDescuento}</p>
                                                <div class="mt-3 custom-button-container">
